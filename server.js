...
const { logSystemToDiscord, logBookingToDiscord, sendNotification, sendNotificationUpdate } = require('./logToDiscord'); // import function ที่แยกไว้
...
app.post('/addCustomerCourse', verifyToken, async (req, res) => {
  try {
    ...
    if (results.affectedRows > 0) {
      //Send Log to Discord
      const logMessage = `สร้าง Customer Course: ${courserefer} โดย ${req.user.username}\n มีรายละเอียดดังนี้:\n` +
...
app.post('/updateCustomerCourse', verifyToken, async (req, res) => {
  try {
    ...
    if (results.affectedRows > 0) {
      //Send Log to Discord
      const logMessage = `แก้ไขข้อมูล Customer Course: ${courserefer} โดย ${req.user.username}\n` +
    ...
    if (resultsUpdateDelete.affectedRows > 0) {
      ...
      if (resultsMoveToHistory.affectedRows > 0) {
        ...
        if (results.affectedRows > 0) {
          ...
          //Send Log to Discord
          const logMessage = `ลบ Customer Course: ${courserefer} โดย ${req.user.username}`;
          await logCourseToDiscord(logMessage);
